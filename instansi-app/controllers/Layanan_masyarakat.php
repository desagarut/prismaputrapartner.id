...
class Layanan_masyarakat extends Mandiri_Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		mandiri_timeout();
		$this->load->model(['web_dokumen_model', 'surat_model', 'penduduk_model', 'keluar_model', 'permohonan_surat_model', 'mailbox_model', 'penduduk_model', 'lapor_model', 'keluarga_model', 'layanan_masyarakat_model', 'anjungan_model','wilayah_model', 'referensi_model','web_rumah_model']);
	...
	public function index()
	{
		...
		{
			redirect('layanan_masyarakat/mandiri/1/1');
		}
	...
	public function auth()
	{
		...
		{
			$this->layanan_masyarakat_model->insidega();
		}

		if ($this->session->lg == 1)
		{
			redirect('layanan_masyarakat/ganti_pin');
		}
	...
	{
		$this->layanan_masyarakat_model->logout();
		redirect('layanan_masyarakat');
	}

	public function update_pin($nik = '')
	{
		$this->layanan_masyarakat_model->update_pin($nik);
		if ($this->session->success == -1)
		{
			redirect($_SERVER['HTTP_REFERER']);
		}
		else redirect('layanan_masyarakat/logout');
	}
	...
	public function ganti_pin()
	{
		if ($this->session->nik)
		{
			...
			$data['main'] = $this->layanan_masyarakat_model->get_penduduk($nik, TRUE);
			$data['header'] = $this->header;
			$data['cek_anjungan'] = $this->cek_anjungan;

			$this->load->view('mandiri_pin', $data);
		}
		else redirect('layanan_masyarakat');
	}
		...
		*/
		switch ($m)
		{
			case 1:
				...
				$data['bantuan_keluarga'] = $this->manajemen_kasus_model->daftar_bantuan_yang_diterima($_SESSION['kk']);
				$data['bantuan_penduduk'] = $this->manajemen_kasus_model->daftar_bantuan_yang_diterima($_SESSION['nik']);
				...
				$data['program'] = $this->manajemen_kasus_model->get_peserta_program(2, $data['kepala_kk']['no_kk']);
			...
			case 7:
				$data['list_polling'] = $this->penduduk_model->list_dokumen($_SESSION['id']);
				break;
		...
		$data['penduduk'] = $this->penduduk_model->get_penduduk($_SESSION['id']);
		$this->load->view('layanan_masyarakat/layouts/layout.mandiri.php', $data);
	}
	...
	public function mandiri_surat($id_permohonan='')
	{
		...
		$this->load->view('layanan_masyarakat/commons/layout.mandiri.php', $data);
	}
	...
	public function kartu_peserta($aksi = 'tampil', $id = 0)
	{
		...
		if ($data['peserta'] == $_SESSION['nik'])
		{
			if ($aksi == 'tampil')
			{
				$this->load->view('layanan_masyarakat/data_peserta', $data);
			}
			else
			{
				$this->load->helper('download');
				...
				redirect('layanan_masyarakat/mandiri/1/4');
			}
		}
	}
	...
	public function cek_syarat()
	{
		...
		foreach ($syarat_surat as $no_syarat => $baris)
		{
			...
			$pilihan_dokumen = $this->load->view('layanan_masyarakat/pilihan_syarat.php', array('dokumen' => $dokumen, 'syarat_permohonan' => $syarat_permohonan, 'syarat_id' => $baris['ref_syarat_id']), TRUE);
			$row[] = $pilihan_dokumen;
			$data[] = $row;
		}
	...
	public function ajax_table_surat_permohonan()
	{
		...
		for ($i=0; $i < count($data); $i++)
		{
			...
			$list_dokumen[$i][] = "<a href='".site_url("layanan_masyarakat/unduh_berkas/".$data[$i][id])."/{$data[$i][id_pend]}"."'>".$data[$i]["nama"].'</a>';
...
// Rumah
	...
	public function mandiri_dokumen($id_permohonan='')
	{
		...
		$this->load->view('layanan_masyarakat/dokumen', $data);
	}
	
	public function mandiri_polling($id = null)
	{
		...
		$this->load->view('layanan_masyarakat/commons/header');
		$this->load->view('layanan_masyarakat/commons/menu');
		$this->load->view('layanan_masyarakat/polling/daftar', [
			'list_tahun' => $this->model->list_filter_tahun(),
		]);
		$this->load->view('layanan_masyarakat/commons/footer');
	}
}
