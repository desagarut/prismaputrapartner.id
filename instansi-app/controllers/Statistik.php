...
class Statistik extends Admin_Controller {
	...
	{
		parent::__construct();
		$this->load->model(['wilayah_model', 'laporan_penduduk_model', 'pamong_model', 'manajemen_kasus_model', 'config_model', 'referensi_model']);
	...
	public function index()
	{
		...
		$data['stat_bantuan'] = $this->manajemen_kasus_model->list_program(0);
	...
	private function get_data_stat(&$data, $lap)
	{
		...
		if ($lap > 50)
		{
			...
			$data['program'] = $this->manajemen_kasus_model->get_sasaran($program_id);
	...
	*/
	...
	{
		$peserta = $this->manajemen_kasus_model->get_peserta_bantuan();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
			"recordsTotal" => $this->manajemen_kasus_model->count_peserta_bantuan_all(),
			"recordsFiltered" => $this->manajemen_kasus_model->count_peserta_bantuan_filtered(),
			'data' => $data
		);
	...
	public function resume()
	{
		...
		$this->render('statistik/resume', $data);
	}
}
