...
class Dokumen_kasus extends Admin_Controller {
    ...
    $_SESSION['menu_kategori'] = TRUE;
		...
		$data['subtitle'] = ($kat == '3') ? "Buku Peraturan Desa" : "Dokumen Putusan Pengadilan";
		$data['selected_nav'] = ($kat == '3') ? 'peraturan' : 'keputusan';

		$this->load->view('header', $this->header);
		$this->load->view('nav', $nav);
		$this->load->view('dokumen_kasus/umum/main', $data);
	...
	public function clear($kat=2)
	{
		...
		redirect("dokumen_kasus/peraturan_desa/$kat");
	}
	...
	public function form($kat=2, $p=1, $o=0, $id='')
	{
		...
		if ($id)
		{
			...
			$data['form_action'] = site_url("dokumen_kasus/update/$kat/$id/$p/$o");
		...
		{
			$data['dokumen'] = null;
			$data['form_action'] = site_url("dokumen_kasus/insert");
		}
		$data['kat_nama'] = $this->web_dokumen_model->kat_nama($kat);
	...
	public function search()
	{
		...
		redirect("dokumen_kasus/index/$kat");
	}
	...
	{
		$this->redirect_hak_akses('h', "dokumen_kasus/index/$kat/$p/$o");
