...
class Penduduk extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		//$this->load->model(['penduduk_model', 'keluarga_model', 'wilayah_model', 'referensi_model', 'web_dokumen_model', 'config_model', 'manajemen_kasus_model', 'lapor_model', 'web_rumah_model']);
		$this->load->model(['penduduk_model', 'keluarga_model', 'wilayah_model', 'referensi_model', 'web_dokumen_model', 'config_model', 'manajemen_kasus_model', 'web_rumah_model']);
		$this->modul_ini = 2;
		$this->sub_modul_ini = 21;
		$this->_set_page = ['50', '100', '200'];
		$this->_list_session = ['filter_tahun', 'filter_bulan', 'status_hanya_tetap', 'jenis_peristiwa', 'filter', 'status_dasar', 'sex', 'agama', 'provinsi', 'kabkota', 'kecamatan', 'desa', 'rw', 'rt', 'cari', 'umur_min', 'umur_max', 'umurx', 'pekerjaan_id', 'status', 'pendidikan_sedang_id', 'pendidikan_kk_id', 'status_penduduk', 'judul_statistik', 'cacat', 'cara_kb_id', 'akta_kelahiran', 'status_ktp', 'id_asuransi', 'manajemen_kasus', 'log', 'warganegara', 'menahun', 'hubungan', 'golongan_darah', 'hamil', 'kumpulan_nik', 'suku', 'bpjs_ketenagakerjaan', 'nik_sementara'];
	}
	...
	public function index($p = 1, $o = 0)
	{
		...
		{
			if (in_array($list, ['provinsi', 'kabkota', 'kecamatan']))
		...
		if (isset($provinsi))
		{
			...
			$data['list_kabkota'] = $this->wilayah_model->list_kabkota($provinsi);

			if (isset($kabkota))
			{
				$data['kabkota'] = $kabkota;
				$data['list_kecamatan'] = $this->wilayah_model->list_kecamatan($provinsi, $kabkota);
		...
		{
			$data['provinsi'] = $data['kabkota'] = $data['kecamatan'] = '';
		}
	...
	public function form($p = 1, $o = 0, $id = '')
	{
		...
		$data['kabkota']			= $this->wilayah_model->list_kabkota($data['penduduk']['provinsi']);
		$data['kecamatan'] 			= $this->wilayah_model->list_kecamatan($data['penduduk']['provinsi'], $data['penduduk']['kabkota']);
		$data['desa'] 				= $this->wilayah_model->list_desa($data['penduduk']['provinsi'], $data['penduduk']['kabkota'], $data['penduduk']['kecamatan']);
		$data['rw'] 				= $this->wilayah_model->list_rw($data['penduduk']['provinsi'], $data['penduduk']['kabkota'], $data['penduduk']['kecamatan'], $data['penduduk']['desa']);
		$data['rt'] 				= $this->wilayah_model->list_rt($data['penduduk']['provinsi'], $data['penduduk']['kabkota'], $data['penduduk']['kecamatan'], $data['penduduk']['desa'], $data['penduduk']['rw']);
	...
	public function detail($p = 1, $o = 0, $id = 0)
	{
		...
		$data['penduduk'] = $this->penduduk_model->get_penduduk($id);
		$data['program'] = $this->manajemen_kasus_model->get_peserta_program(1, $data['penduduk']['nik']);
	...
	{
		if ($filter == "provinsi") $this->session->unset_userdata(['kabkota', 'kecamatan', 'desa', 'rw', 'rt']);
		if ($filter == "kabkota") $this->session->unset_userdata(['kecamatan', 'desa', 'rw', 'rt']);
		if ($filter == "kecamatan") $this->session->unset_userdata(['desa', 'rw', 'rt']);
		if ($filter == "desa") $this->session->unset_userdata(['rw', 'rt']);
	...
	public function ajax_penduduk_pindah_rw($desa = '')
	{
		$desa = urldecode($desa);
		$rw = $this->wilayah_model->list_rw($desa);
		echo "<div class='form-group'><label>RW</label>
		<select class='form-control input-sm' name='rw' onchange=RWSel('" . rawurlencode($desa) . "',this.value)>
	...
	public function ajax_penduduk_pindah_rt($desa = '', $rw = '')
	{
		$desa = urldecode($desa);
		$rt = $this->wilayah_model->list_rt($desa, $rw);
	...
	public function ajax_penduduk_cari_rw($desa = '')
	{
		$rw = $this->wilayah_model->list_rw($desa);
		echo "<td>RW</td>
		<td><select name='rw' onchange=RWSel('" . $desa . "',this.value)>
		<option value=''>Pilih RW&nbsp;</option>";
		foreach ($rw as $data) {
			echo "<option>" . $data['rw'] . "</option>";
		}
	...
	public function ajax_penduduk_maps_koordinat($p = 1, $o = 0, $id = '', $edit = '')
	{
		$data['p'] = $p;
		$data['o'] = $o;
		$data['id'] = $id;
		...
		$data['provinsi_gis'] = $this->wilayah_model->list_provinsi();
		$data['kabkota_gis'] = $this->wilayah_model->list_kabkota();
		$data['kecamatan_gis'] = $this->wilayah_model->list_kecmatan();
		$data['desa_gis'] = $this->wilayah_model->list_desa();
		$data['rw_gis'] = $this->wilayah_model->list_rw_gis();
		$data['rt_gis'] = $this->wilayah_model->list_rt_gis();
		$this->load->view('sid/kependudukan/ajax_penduduk_maps_koordinat', $data);
	}

	public function ajax_penduduk_maps_openstreet($p = 1, $o = 0, $id = '', $edit = '')
	{
		$data['p'] = $p;
		$data['o'] = $o;
		$data['id'] = $id;
		...
		$this->render('sid/kependudukan/ajax_penduduk_maps_openstreet', $data);
		//$this->render('sid/kependudukan/penduduk_rumah', $data);
	}

	public function ajax_penduduk_maps_google($p = 1, $o = 0, $id = '', $edit = '')
	{
		$data['p'] = $p;
		$data['o'] = $o;
		$data['id'] = $id;
		...
		$data['rt_gis'] = $this->wilayah_model->list_rt_gis();
		$data['form_action'] = site_url("penduduk/update_maps/$p/$o/$id/$edit");
		$this->load->view('sid/kependudukan/ajax_penduduk_maps_google', $data);
	}
	...
	public function update_maps($p = 1, $o = 0, $id = '', $edit = '')
	{
		...
		if ($edit == 1)
			redirect("penduduk/detail/$p/$o/$id");
		else
			redirect("penduduk");
	}
	...
	public function statistik($tipe = '0', $nomor = 0, $sex = NULL)
	{
		...
		switch ($tipe) {
			...
			case 'bantuan_penduduk':
				$session = 'manajemen_kasus';
				$kategori = 'MANAJEMEN KASUS : ';
				break;
			case 18:
				if ($sex == NULL) {
	...
	public function lap_statistik($id_cluster = 0, $tipe = 0, $nomor = 0)
	{
		...
		switch ($tipe) {
			case 1:
				$_SESSION['sex'] = '1';
				$_SESSION['provinsi'] = $cluster['provinsi'];
				$_SESSION['kabkota'] = $cluster['kabkota'];
				$_SESSION['kecamatan'] = $cluster['kecamatan'];
				$_SESSION['desa'] = $cluster['desa'];
