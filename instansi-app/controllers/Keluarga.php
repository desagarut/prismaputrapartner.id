...
class Keluarga extends Admin_Controller {
	...
	{
		parent::__construct();
		$this->load->model(['keluarga_model', 'penduduk_model', 'wilayah_model', 'manajemen_kasus_model', 'referensi_model', 'config_model']);
		$this->modul_ini = 2;
		$this->sub_modul_ini = 22;
		$this->_set_page = ['20', '50', '100'];
		$this->_list_session = ['status_dasar', 'sex', 'provinsi', 'rw', 'rt', 'cari', 'kelas', 'filter', 'id_bos', 'judul_statistik', 'bantuan_keluarga', 'kumpulan_kk'];
	}
	...
	public function form($p = 1, $o = 0)
	{
		...
		$data['kabkota'] = $this->wilayah_model->list_kabkota($data['penduduk']['provinsi'], $data['penduduk']['kabkota']);
		$data['kecamatan'] = $this->wilayah_model->list_kecamatan($data['penduduk']['provinsi'], $data['penduduk']['kecamatan']);
	...
	public function edit_nokk($p = 1, $o = 0, $id = 0)
	{
		...
		$data['provinsi'] = $this->wilayah_model->list_provinsi();
		$data['rw'] = $this->wilayah_model->list_rw($data['kk']['provinsi']);
		$data['rt'] = $this->wilayah_model->list_rt($data['kk']['provinsi'], $data['kk']['rw']);
		$data['program'] = $this->manajemen_kasus_model->list_program_keluarga($id);
	...
	public function provinsi()
	{
		...
		if ($provinsi != "")
			$this->session->provinsi = $provinsi;
		else $this->session->unset_userdata('provinsi');
		redirect('keluarga');
	}
