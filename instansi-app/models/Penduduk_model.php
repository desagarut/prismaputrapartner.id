...
class Penduduk_model extends MY_Model
{
	...
	protected function provinsi_sql()
	{
		if (!empty($_SESSION['provinsi'])) {
			$kf = $_SESSION['provinsi'];
			$provinsi_sql = " AND ((u.id_kk <> '0' AND a.provinsi = '$kf') OR (u.id_kk = '0' AND a2.provinsi = '$kf'))";
			return $provinsi_sql;
		}
	}

	protected function kabkota_sql()
	{
		if (!empty($_SESSION['kabkota'])) {
			$kf = $_SESSION['kabkota'];
			$kabkota_sql = " AND ((u.id_kk <> '0' AND a.kabkota = '$kf') OR (u.id_kk = '0' AND a2.kabkota = '$kf'))";
			return $kabkota_sql;
		}
	}

	protected function kecamatan_sql()
	{
		if (!empty($_SESSION['kecamatan'])) {
			$kf = $_SESSION['kecamatan'];
			$kecamatan_sql = " AND ((u.id_kk <> '0' AND a.kecamatan = '$kf') OR (u.id_kk = '0' AND a2.kecamatan = '$kf'))";
			return $kecamatan_sql;
		}
	}

	protected function desa_sql()
	{
		if (!empty($_SESSION['desa'])) {
			$kf = $_SESSION['desa'];
			$desa_sql = " AND ((u.id_kk <> '0' AND a.desa = '$kf') OR (u.id_kk = '0' AND a2.desa = '$kf'))";
			return $desa_sql;
		}
	}
	...
	public function get_alamat_wilayah($id)
	{
		...
		$sql = "SELECT a.provinsi, a.kabkota, a.kecamatan, a.desa, a.rw, a.rt, u.alamat_sekarang as alamat
		...
		$alamat_wilayah = trim("$data[alamat] RT $data[rt] / RW $data[rw] / desa $data[desa] / kecamatan $data[kecamatan] / kabkota $data[kabkota]" . ikut_case($data['provinsi'], $this->setting->sebutan_provinsi) . " $data[provinsi]");
	...
	private function list_data_sql()
	{
		...
		$sql .= $this->provinsi_sql();
		$sql .= $this->kabkota_sql();
		$sql .= $this->kecamatan_sql();
		$sql .= $this->desa_sql();
		$sql .= $this->rw_sql();
		$sql .= $this->rt_sql();
	...
	public function list_data($o = 1, $offset = 0, $limit = 0)
	{
		...
		$select_sql .= "u.id, u.nik, u.tanggallahir, u.tempatlahir, u.foto, u.status, u.status_dasar, u.id_kk, u.nama, u.nama_ayah, u.nama_ibu, a.provinsi, a.kabkota, a.kecamatan, a.desa, a.rw, a.rt, d.alamat, d.no_kk AS no_kk, u.kk_level, u.tag_id_card, u.created_at, u.created_at,
			(CASE
			...
			x.nama AS sex, sd.nama AS pendidikan_sedang, n.nama AS pendidikan, p.nama AS pekerjaan, g.nama AS agama, m.nama AS gol_darah, hub.nama AS hubungan, b.no_kk AS no_rtm, b.id AS id_rtm, ux.nama as nama_pengubah, ucreate.nama as nama_pendaftar, u.sex as id_sex
		";
		//Main Query
		$list_data_sql = $this->list_data_sql();
		...
		switch ($o) {
			case 1:
				$order_sql = ' ORDER BY u.nik';
				break;
			case 2:
				$order_sql = ' ORDER BY u.nik DESC';
				break;
			case 3:
			...
			case 4:
				...
				break;
			case 5:
				$order_sql = ' ORDER BY CONCAT(d.no_kk, u.kk_level)';
				break;
			case 6:
				$order_sql = ' ORDER BY d.no_kk DESC, u.kk_level';
				break;
			case 7:
				$order_sql = ' ORDER BY umur';
				break;
			case 8:
				$order_sql = ' ORDER BY umur DESC';
				break;
			case 9:
			...
			case 10:
				...
				break;
			case 11:
				$order_sql = ' ORDER BY created_by';
				break;
			case 12:
				$order_sql = ' ORDER BY created_by DESC';
				break;
			default:
				$order_sql = ' ORDER BY CONCAT(d.no_kk, u.kk_level)';
		}
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			if (!$data[$i]['id_kk'] or $data[$i]['id_kk'] == 0) {
				...
				$sql = "SELECT p.id_cluster, p.alamat_sekarang, c.provinsi, c.kabkota, c.kecamatan, c.desa, c.rw, c.rt
				...
				$data[$i]['provinsi'] = $penduduk['provinsi'];
				$data[$i]['kabkota'] = $penduduk['kabkota'];
				$data[$i]['kecamatan'] = $penduduk['kecamatan'];
				$data[$i]['desa'] = $penduduk['desa'];
	...
	{
		//Main Query
		$sql = "SELECT u.id, u.nik, u.nama, map.lat, map.lng, a.provinsi, a.kabkota, a.kecamatan, a.desa, a.rw, a.rt, u.foto, d.no_kk AS no_kk,
				...
				@alamat:=trim(concat_ws('',
					...
					case
						when a.desa != '-' then concat('Desa ', a.desa)
						else ''
					end,
					case
						when a.kecamatan != '-' then concat('Kecamatan ', a.kecamatan)
						else ''
					end,
					case
						when a.kabkota != '-' then concat('Kabupaten/Kota ', a.kabkota)
						else ''
					end,
					case
						when a.provinsi != '-' then concat('Provinsi ', a.provinsi)
						else ''
					end
				)),
				case
	...
	{
		$sql = "SELECT u.sex as id_sex, u.*, a.provinsi, a.kabkota, a.kecamatan, a.desa, a.rw, a.rt, t.id AS id_status, t.nama AS status, o.nama AS pendidikan_sedang, m.nama as golongan_darah, h.nama as hubungan,
		...
		if ($data['id_kk'] == 0 or $data['id_kk'] == '') {
			...
			$data['provinsi'] = $cluster['provinsi'];
			$data['kabkota'] = $cluster['kabkota'];
			$data['kecamatan'] = $cluster['kecamatan'];
			$data['desa'] = $cluster['desa'];
			$data['rw'] = $cluster['rw'];
			$data['rt'] = $cluster['rt'];
		}
	...
	public function get_penduduk_by_nik($nik = 0)
	{
		...
		w.nama AS status_kawin, f.nama AS warganegara, a.nama AS agama, h.nama as hubungan, d.nama AS pendidikan, j.nama AS pekerjaan, u.nik AS nik, c.rt AS rt, c.rw AS rw, c.desa AS desa, c.kecamatan AS kecamatan, c.kabkota AS kabkota, c.provinsi AS provinsi, k.no_kk AS no_kk, k.alamat,
		...
		$data = $query->row_array();
		$data['alamat_wilayah'] = trim("$data[alamat] RT $data[rt] / RW $data[rw] $data[desa]  $data[kecamatan] $data[kabkota] $data[provinsi]");
		return $data;
	}
	...
	{
		$sql = "SELECT * FROM tweb_wilayah WHERE kabkota = '0' AND kecamatan = '0' AND desa = '0'  AND rw = '0' AND rt = '0'";
		$query = $this->db->query($sql);
		$data = $query->result_array();
		return $data;
	}
...
*/
	...
	public function list_penduduk_status_dasar($status_dasar = 1)
	{
		$sql = "SELECT u.id, nik, nama,
			CONCAT('Alamat : RT-', w.rt, ', RW-', w.rw,'', w.desa,'', w.kecamatan,'', w.kabkota,'', w.provinsi) AS alamat,
			CONCAT('NIK: ', nik, ' - ', nama, '\nAlamat : RT-', w.rt, ', RW-', w.rw, '', w.desa,'', w.kecamatan,'', w.kabkota,'', w.provinsi) AS info_pilihan_penduduk,
			w.rt, w.rw, w.desa, w.kecamatan, w.kabkota, w.provinsi, u.sex FROM tweb_penduduk u LEFT JOIN tweb_wilayah w ON u.id_cluster = w.id WHERE u.status_dasar = ?";
	...
	{
		$jml = $this->db->select('count(*) as jml')->where('status', '1')->get('tweb_penduduk')->row()->jml;
		return $jml;
	}
