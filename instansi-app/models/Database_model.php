...
class Database_model extends CI_Model {
	...
	public function migrasi_db_cri()
	{
		...
		$this->folder_instansi_model->amankan_folder_desa();
  ...
  private function migrasi_1812_ke_1901()
  {
  	...
  	// Tambah kolom tweb_pamong
  	if (!$this->db->field_exists('pamong_pangkat', 'tweb_pamong'))
  	{
			// Tambah kolom
			$fields = array();
			...
			$this->dbforge->add_column('tweb_pamong', $fields);
  	}

  	// Urut tabel tweb_pamong
  	if (!$this->db->field_exists('urut', 'tweb_pamong'))
  	{
			// Tambah kolom
			$fields = array();
			$fields['urut'] = array(
  ...
  {
  	// Ubah struktur tabel tweb_pamong
  	if (!$this->db->field_exists('id_pend', 'tweb_pamong'))
  	{
			// Tambah kolom
			$fields = array();
  ...
  private function migrasi_1809_ke_1810()
  {
  	...
  	// Tambah klasifikasi surat
		...
		// sesuai dengan issue https://github.com/sidega/sidega/issues/999
		...
		// buat tabel disposisi dengan relasi ke surat masuk dan tweb_pamong
		if (!$this->db->table_exists('disposisi_surat_masuk'))
		{
			...
			$this->dbforge->add_column('disposisi_surat_masuk', array(
		    	...
		    	'CONSTRAINT `desa_pamong_fk` FOREIGN KEY (`id_desa_pamong`) REFERENCES `tweb_pamong` (`pamong_id`) ON DELETE CASCADE ON UPDATE CASCADE'
			));
  ...
  private function migrasi_1808_ke_1809()
  {
	...
	if (!$this->db->field_exists('parent', 'setting_modul') or strpos($this->getCurrentVersion(), '18.08') !== false)
	{
	  ...
	  $query = "
		...
		('1', 'Home', 'beranda', '1', 'fa-home', '1', '2', '0', '1', 'fa fa-home'),
  ...
  private function migrasi_1806_ke_1807()
  {
	...
	private function migrasi_25_ke_26()
	{
		...
		$query = "
			...
			ON DUPLICATE KEY UPDATE
				...
				url = VALUES(url)";
		$this->db->query($query);
		// Tambah folder instansi/upload/media
		if (!file_exists('/instansi/upload/media'))
		{
			mkdir('instansi/upload/media');
			xcopy('instansi-contoh/upload/media', 'instansi/upload/media');
		}
		if (!file_exists('/instansi/upload/thumbs'))
		{
			mkdir('instansi/upload/thumbs');
			xcopy('instansi-contoh/upload/thumbs', 'instansi/upload/thumbs');
		}
	...
	private function migrasi_115_ke_116()
	{
		...
		if (!$this->db->field_exists('pamong_ttd', 'tweb_pamong')) {
			$query = "ALTER TABLE tweb_pamong ADD pamong_ttd tinyint(1)";
			$this->db->query($query);
		}
	...
	{
		// Buat folder instansi/upload/pengesahan apabila belum ada
	...
	private function migrasi_111_ke_112()
	{
		...
		if (!$this->db->field_exists('jenis', 'tweb_surat_format'))
		{
			...
			$this->db->query($query);
			// Update semua surat yang disediakan oleh rilis sidega
			$surat_sistem = array(
				'surat_ket_pengantar',
				'surat_ket_penduduk',
				'surat_bio_penduduk',
