...
class MY_Controller extends CI_Controller {
	...
	function set_template($template_file = 'template.php')
	{
		...
		if (is_file($template_file_path))
			...
			$this->template = '../../themes/lawmaker/' . $template_file;
	}
	...
	{
		$cluster_session = array('provinsi', 'kabkota', 'kecamatan', 'desa', 'rw', 'rt');
...
class Web_Controller extends MY_Controller {
	...
	public static function fallback_default($theme, $view)
	{
		...
		{
			$theme_view = "../../themes/lawmaker/$view";
		}
...
class Admin_Controller extends MY_Controller {
	...
	protected function redirect_hak_akses($akses, $redirect = '', $controller = '')
	{
		...
		{
			session_error("Anda tidak mempunyai akses pada fitur ini");
			if (empty($this->grup)) redirect('insidega');
