<div class="content-wrapper">
	...
	<div class="content-header">
		<div class="container-fluid">
			<div class="row mb-2">
				<div class="col-sm-6">
					<h5 class="m-0">Dokumentasi Kegiatan</h5>
				</div>
							...
							<table class="table table-bordered table-striped table-hover">
								<tbody>
									...
									<tr>
										...
										<td nowrap> : <?= strtoupper($this->setting->sebutan_dusun) ?> : <?= $penduduk['dusun'] ?> <?= $penduduk['alamat'] ?> RW/RT : <?= $penduduk['rw'] ?>/<?= $penduduk['rt'] ?> <?= $penduduk['alamat'] ?> </td>
									</tr>
								</tbody>
							</table>
						</div>
														...
														<tbody>
															<?php foreach ($list_rumah as $data): ?>
																<tr>
																	...
																	<td><?= $key + 1 ?></td>
																	<td nowrap>
																		<a href="<?= base_url() . LOKASI_RUMAH ?><?= urlencode($data['satuan']) ?>" class="btn bg-info btn-box btn-sm" rel=”noopener noreferrer” target="_blank" title="Buka"><i class="fa fa-eye"></i></a>
																		<?php if (!$data['hidden']): ?>
