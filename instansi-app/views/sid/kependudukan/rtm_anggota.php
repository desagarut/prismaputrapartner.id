...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h4 class="m-0">Daftar Anggota Rumah Tangga</h4>
        </div>
        <!-- /.col -->
        <div class="col-sm-6">
	...
	<section class="content" id="maincontent">
		<div class="card card-info">
			<div class="card-header">
				...
				<a href="#confirm-delete" title="Hapus Data" onclick="deleteAllBox('mainform','<?= site_url("rtm/delete_all_anggota/$kk")?>')" class="btn btn-social btn-box	btn-danger btn-sm visible-xs-block visible-sm-inline-block visible-md-inline-block visible-lg-inline-block hapus-terpilih"><i class='fa fa-trash'></i> Hapus Data Terpilih</a>
				<?php endif;?>
				...
				<div class="table-responsive">
					<table class="table table-bordered table-striped table-hover tabel-rincian">
						<tbody>
							...
							<tr>
								<td>
									<?= ($program['programkerja']) ? anchor("program_tambahan/peserta/3/$kepala_kk[no_kk]", 'Program Tambahan', 'target="_blank"') : 'Program Tambahan'; ?>
								</td>
								<td>:</td>
								<td>
									<?php if($program['programkerja']): ?>
										<?php foreach ($program['programkerja'] as $item): ?>
											<?= anchor("program_tambahan/data_peserta/$item[peserta_id]", '<span class="label label-success">' . $item['nama'] . '</span>&nbsp;', 'target="_blank"'); ?>
										<?php endforeach; ?>
									<?php else: ?>
										-
									<?php endif; ?>
									...
									<?php foreach ($main as $key => $data): ?>
										<tr>
                                            ...
                                            <td class="aksi">
												...
												<a href="#" data-href="<?= site_url("rtm/delete_anggota/$kk/$data[id]")?>" class="btn bg-maroon btn-box btn-sm" title="Hapus Data" data-toggle="modal" data-target="#confirm-delete"><i class="fa fa-trash"></i></a>
											</td>
                                            <?php endif;?>
											<td><?= $data['nik']?></td>
											<td><?= $data['no_kk']?></td>
